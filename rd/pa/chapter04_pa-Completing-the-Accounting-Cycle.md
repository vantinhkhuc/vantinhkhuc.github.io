# SCRIPT BÀI GIẢNG: HOÀN THÀNH CHU TRÌNH KẾ TOÁN
## Completing the Accounting Cycle

**Môn học:** Nguyên Lý Kế Toán  
**Đối tượng:** Sinh viên năm 2 - Ngành Thương mại điện tử  
**Thời lượng:** 3 buổi × 150 phút = 450 phút (7.5 giờ)  
**Tài liệu tham khảo:** Chapter 4 - Intermediate Accounting (IFRS Edition)

---

# 📅 BUỔI HỌC 1: WORKSHEET (150 phút)

## I. GIỚI THIỆU VÀ ĐỘNG VIÊN (10 phút)

### Slide 1-2: Mở đầu

**Script:**

> Chào các em! Hôm nay chúng ta bắt đầu Chương 4 - một chương rất quan trọng về "Hoàn thành Chu trình Kế toán". 
>
> Các em đã học qua Chương 3 về điều chỉnh cuối kỳ. Vậy sau khi điều chỉnh xong, chúng ta làm gì tiếp theo? Đó chính là nội dung của chương này.
>
> **[Tương tác]** Các em có biết tại sao chu trình kế toán lại được gọi là "chu trình" không? Đúng rồi! Vì nó lặp đi lặp lại mỗi kỳ kế toán, giống như một vòng tròn không có điểm kết thúc.

### Slide 3-4: Mục tiêu học tập

**Script:**

> Trong 3 buổi học này, chúng ta sẽ học 4 phần chính:
>
> **Buổi 1 hôm nay:** Tập trung vào WORKSHEET - một công cụ rất hữu ích giúp các em tổ chức công việc kế toán cuối kỳ.
>
> **Buổi 2:** Chúng ta sẽ học về CLOSING ENTRIES - cách "đóng sổ" các tài khoản để chuẩn bị cho kỳ kế toán mới.
>
> **Buổi 3:** Tổng hợp toàn bộ chu trình kế toán và học cách sửa sai sót, cùng với cách trình bày Bảng cân đối kế toán chuyên nghiệp.
>
> Sau khi học xong chương này, các em sẽ có cái nhìn toàn cảnh về quy trình kế toán từ đầu đến cuối. Đây là kiến thức nền tảng rất quan trọng cho những môn kế toán chuyên sâu sau này.

---

## II. PHẦN 1: WORKSHEET (120 phút)

### Slide 5-6: Khái niệm Worksheet (15 phút)

**Script:**

> **[Chuyển slide - Phần 1]**
>
> Bây giờ chúng ta bắt đầu với phần đầu tiên: WORKSHEET hay còn gọi là Bảng tính kế toán.
>
> **[Hỏi lớp]** Có ai trong các em đã từng nghe đến hoặc làm việc với worksheet chưa? Có thể là trong Excel?
>
> Đúng vậy! Worksheet trong kế toán rất giống với spreadsheet trong Excel. Nó là một bảng có nhiều cột, giúp chúng ta tổ chức và xử lý thông tin một cách có hệ thống.
>
> **Định nghĩa:** Worksheet là một biểu mẫu nhiều cột được sử dụng trong quá trình điều chỉnh và chuẩn bị báo cáo tài chính.
>
> **[Nhấn mạnh]** Các em cần nhớ 3 điểm quan trọng về worksheet:
>
> 1. Nó CHỈ là công cụ làm việc - không phải là sổ sách chính thức
> 2. Nó KHÔNG phải là phần của sổ cái
> 3. Sử dụng worksheet là TÙY CHỌN, không bắt buộc
>
> Vậy tại sao chúng ta vẫn học về nó? Vì worksheet giúp chúng ta:
> - Tổ chức công việc tốt hơn
> - Giảm thiểu sai sót
> - Lập báo cáo tài chính nhanh hơn - TRƯỚC KHI ghi nhật ký và chuyển sổ các bút toán điều chỉnh
>
> Trong thực tế, nhiều công ty nhỏ và vừa vẫn sử dụng worksheet, thường bằng Excel hoặc Google Sheets.

### Slide 7: Cấu trúc Worksheet (10 phút)

**Script:**

> Bây giờ chúng ta xem worksheet có cấu trúc như thế nào.
>
> **[Chỉ vào slide]** Worksheet có 10 cột, được chia thành 5 phần chính. Mỗi phần có 2 cột: Debit (Nợ) và Credit (Có).
>
> **1. Account Titles:** Cột đầu tiên - liệt kê tên các tài khoản
>
> **2. Trial Balance:** Bảng cân đối số phát sinh ban đầu - số liệu từ sổ cái TRƯỚC KHI điều chỉnh
>
> **3. Adjustments:** Các bút toán điều chỉnh - chúng ta đã học ở Chương 3
>
> **4. Adjusted Trial Balance:** Số dư sau khi đã điều chỉnh
>
> **5. Income Statement:** Kéo các tài khoản doanh thu và chi phí vào đây
>
> **6. Balance Sheet (Statement of Financial Position):** Kéo các tài khoản tài sản, nợ, vốn vào đây
>
> **[Lưu ý]** Các em cần nhớ: mỗi phần đều có 2 cột Debit và Credit. Đây là nguyên tắc ghi sổ kép mà chúng ta đã học.

### Slide 8-13: 5 Bước chuẩn bị Worksheet (40 phút)

**Script - Bước 1:**

> Bây giờ chúng ta đi vào chi tiết 5 bước chuẩn bị worksheet. Các em nên ghi chú cẩn thận phần này vì đây là quy trình chuẩn các em sẽ áp dụng trong bài tập.
>
> **BƯỚC 1: PREPARE TRIAL BALANCE**
>
> Bước đầu tiên rất đơn giản: Chúng ta lấy tất cả các tài khoản có số dư từ sổ cái, nhập vào cột Trial Balance.
>
> **[Nhấn mạnh]** Lưu ý:
> - Phải liệt kê TẤT CẢ tài khoản có số dư
> - Số dư Nợ → cột Debit
> - Số dư Có → cột Credit
> - Tổng Debit = Tổng Credit (phải cân bằng!)
>
> **[Ví dụ]** Công ty Yazici Advertising vào ngày 31/10/2020 có:
> - Cash: 15,200 (Debit)
> - Supplies: 2,500 (Debit)
> - Service Revenue: 10,000 (Credit)
> - ...và các tài khoản khác
>
> Tổng cả hai cột đều là 28,700 - cân bằng hoàn hảo!
>
> **[Hỏi lớp]** Nếu trial balance không cân bằng thì sao? Đúng rồi, phải quay lại kiểm tra sổ cái, chắc chắn có sai sót ở đâu đó.

**Script - Bước 2:**

> **BƯỚC 2: ENTER ADJUSTMENTS**
>
> Đây là bước quan trọng nhất. Chúng ta sẽ nhập các bút toán điều chỉnh vào cột Adjustments.
>
> **Quy tắc:**
> 1. Sử dụng các tài khoản có sẵn trong trial balance
> 2. Nếu cần thêm tài khoản mới → thêm vào dưới trial balance
> 3. Đánh dấu mỗi bút toán bằng chữ cái (a, b, c, d...)
> 4. Mỗi bút toán phải cân bằng (Debit = Credit)
>
> **[Quan trọng]** Các em chưa ghi nhật ký các bút toán này đâu nhé! Chỉ ghi trên worksheet thôi. Sau khi lập xong báo cáo tài chính, chúng ta mới ghi nhật ký.
>
> **Ví dụ với Yazici Advertising:**
> - (a) Supplies used: 1,500
>   - Debit: Supplies Expense 1,500
>   - Credit: Supplies 1,500
>
> - (b) Insurance expired: 50
>   - Debit: Insurance Expense 50
>   - Credit: Prepaid Insurance 50
>
> - (c) Depreciation: 40
> - (d) Unearned revenue earned: 400
> - (e) Accrued revenue: 200
> - (f) Interest accrued: 50
> - (g) Salaries accrued: 1,200
>
> Tổng cột Adjustments: Debit = Credit = 3,440 ✓

**Script - Bước 3:**

> **BƯỚC 3: ENTER ADJUSTED BALANCES**
>
> Bước này chúng ta sẽ tính toán. Công thức rất đơn giản:
>
> **Adjusted Balance = Trial Balance ± Adjustments**
>
> **Quy tắc tính:**
> - **Với tài khoản Debit:**
>   - Cộng các adjustment ở cột Debit
>   - Trừ các adjustment ở cột Credit
>
> - **Với tài khoản Credit:**
>   - Cộng các adjustment ở cột Credit
>   - Trừ các adjustment ở cột Debit
>
> **[Ví dụ cụ thể]**
>
> **Supplies:**
> - Trial Balance: 2,500 Debit
> - Adjustment: 1,500 Credit (dùng đi)
> - Adjusted: 2,500 - 1,500 = 1,000 Debit ✓
>
> **Service Revenue:**
> - Trial Balance: 10,000 Credit
> - Adjustments: 400 Cr + 200 Cr = 600 Credit
> - Adjusted: 10,000 + 600 = 10,600 Credit ✓
>
> **[Kiểm tra]** Sau khi tính xong, các em phải kiểm tra: Tổng Debit = Tổng Credit trong cột Adjusted Trial Balance.
>
> Với Yazici: 30,190 = 30,190 ✓

**Script - Bước 4:**

> **BƯỚC 4: EXTEND TO FINANCIAL STATEMENTS**
>
> Bước này chúng ta "kéo dài" số dư từ Adjusted Trial Balance vào 2 nhóm cột báo cáo tài chính.
>
> **[Nguyên tắc quan trọng]** Mỗi số dư phải được kéo vào MỘT trong BỐN cột:
>
> **Vào INCOME STATEMENT:**
> - Revenue accounts → Credit column
> - Expense accounts → Debit column
>
> **Vào BALANCE SHEET:**
> - Assets → Debit column
> - Liabilities → Credit column
> - Equity → Credit column
> - Dividends → Debit column (vì làm giảm vốn)
>
> **[Lưu ý đặc biệt]** Các em hay nhầm:
> - Accumulated Depreciation → Balance Sheet CREDIT (vì là contra asset)
> - Dividends → Balance Sheet DEBIT (không phải expense!)
>
> **[Mẹo nhớ]** Nếu không biết cho vào đâu, hãy tự hỏi:
> - Đây có phải là doanh thu hoặc chi phí? → Income Statement
> - Còn lại tất cả → Balance Sheet

**Script - Bước 5:**

> **BƯỚC 5: TOTAL AND COMPLETE**
>
> Đây là bước cuối cùng - cũng là lúc chúng ta xác định công ty lãi hay lỗ!
>
> **Các bước cụ thể:**
>
> **1. Tính tổng các cột IS và BS**
> - Income Statement: Cột Debit và Credit
> - Balance Sheet: Cột Debit và Credit
>
> **2. Tính Net Income hoặc Net Loss:**
>
> Net Income = Total Credits (IS) - Total Debits (IS)
>
> Với Yazici:
> - IS Credits: 10,600 (doanh thu)
> - IS Debits: 7,740 (chi phí)
> - Net Income: 10,600 - 7,740 = **2,860** ✓
>
> **3. Nhập Net Income vào worksheet:**
>
> **[Quan trọng]** Nếu có LÃI:
> - Income Statement → Debit (để cân bằng)
> - Balance Sheet → Credit (tăng vốn CSH)
>
> Nếu có LỖ thì ngược lại!
>
> **4. Tính lại tổng:**
> - IS: 10,600 = 10,600 ✓
> - BS: 22,450 = 22,450 ✓
>
> Hoàn thành! Worksheet của chúng ta đã cân bằng hoàn hảo.

### Slide 14-15: Ví dụ hoàn chỉnh (10 phút)

**Script:**

> Các em xem ví dụ hoàn chỉnh của Yazici Advertising trên slide. 
>
> **[Chỉ từng phần]**
> - Trial Balance: 28,700 = 28,700
> - Adjustments: 3,440 = 3,440
> - Adjusted TB: 30,190 = 30,190
> - Income Statement cân bằng sau khi thêm Net Income
> - Balance Sheet cân bằng sau khi thêm Net Income
>
> Các em thấy không? Mỗi bước đều phải cân bằng. Đây là cách chúng ta tự kiểm tra công việc của mình.
>
> **[Thực hành ngay]** Các em hãy dành 2 phút xem kỹ worksheet này, nếu có câu hỏi gì thì giơ tay nhé!

### Slide 16-18: Lập BCTC từ Worksheet (15 phút)

**Script:**

> Sau khi hoàn thành worksheet, chúng ta có thể lập báo cáo tài chính rất nhanh!
>
> **Ba báo cáo chính:**
>
> **1. Income Statement:**
> - Lấy dữ liệu từ cột Income Statement
> - Revenues: 10,600
> - Expenses: 7,740
> - Net Income: 2,860
>
> **2. Retained Earnings Statement:**
> - Beginning balance: 0 (công ty mới)
> - Add: Net Income 2,860
> - Less: Dividends 500
> - Ending: 2,360
>
> **3. Balance Sheet:**
> - Lấy từ cột Balance Sheet
> - Lưu ý: Retained Earnings phải dùng số cuối cùng từ Retained Earnings Statement (2,360)
>
> **[Nhấn mạnh]** Các em nhớ:
> - Worksheet KHÔNG phải là báo cáo tài chính chính thức
> - Định dạng khác nhau
> - Worksheet là nội bộ, BCTC là cho bên ngoài
>
> **[Câu hỏi kiểm tra]** Vậy sau khi lập BCTC từ worksheet xong, chúng ta làm gì tiếp? 
>
> → Đúng rồi! Phải ghi nhật ký và chuyển sổ các bút toán điều chỉnh. Đây là bước không thể bỏ qua!

### Slide 19-20: Bài tập thực hành + Đáp án (20 phút)

**Script:**

> Bây giờ đến phần thực hành! Các em sẽ có 12 phút để làm bài tập này theo nhóm 2-3 người.
>
> **[Đọc đề bài]** Công ty ABC có Trial Balance và 4 adjustment cần thực hiện...
>
> **Yêu cầu:** Lập Worksheet đầy đủ và xác định Net Income.
>
> **[Hướng dẫn]**
> - Bước 1: Ghi Trial Balance
> - Bước 2: Nhập adjustments với chữ cái (a, b, c, d)
> - Bước 3: Tính Adjusted TB
> - Bước 4: Kéo vào IS và BS
> - Bước 5: Tính Net Income
>
> Các em hãy làm việc nhóm, thảo luận với nhau. Sau 12 phút chúng ta sẽ cùng chữa bài.
>
> **[Sau 12 phút]**
>
> Được rồi, chúng ta cùng xem đáp án!
>
> **[Chữa bài chi tiết]**
> - Adjustments: (a) 2,000, (b) 400, (c) 500, (d) 600
> - Adjusted TB cân bằng tại 39,100
> - Net Income = 18,000 - 9,300 = 8,700
>
> Có nhóm nào tính được Net Income = 8,700 không? Rất tốt!
>
> **[Giải thích sai sót thường gặp]**
> - Quên trừ khi có adjustment ở cột ngược lại
> - Kéo sai tài khoản vào IS/BS
> - Đặt Net Income sai vị trí

---

## III. TỔNG KẾT VÀ GIAO BÀI TẬP (10 phút)

**Script:**

> Tốt lắm các em! Chúng ta vừa hoàn thành phần Worksheet - phần đầu tiên của Chương 4.
>
> **Tóm tắt buổi học hôm nay:**
> 1. Worksheet là công cụ hỗ trợ, không bắt buộc
> 2. 5 bước chuẩn bị phải thực hiện tuần tự
> 3. Mỗi bước đều phải cân bằng Debit = Credit
> 4. Từ worksheet, chúng ta lập BCTC rất nhanh
> 5. Vẫn phải ghi nhật ký adjusting entries sau khi lập BCTC
>
> **Bài tập về nhà:**
> - Exercises E4.1, E4.2, E4.3 trong sách giáo khoa
> - Làm thêm worksheet cho công ty DEF (tài liệu trên LMS)
> - Chuẩn bị cho buổi sau: đọc trước về Closing Entries
>
> **Buổi sau:** Chúng ta sẽ học về Closing Entries - cách "khóa sổ" các tài khoản cuối kỳ. Đây là một phần rất quan trọng và thú vị!
>
> Có câu hỏi gì không các em? Hẹn gặp lại các em buổi sau!

---

# 📅 BUỔI HỌC 2: CLOSING ENTRIES (150 phút)

## I. REVIEW VÀ GIỚI THIỆU (10 phút)

**Script:**

> Chào mừng các em đến với buổi học thứ 2 về Chu trình Kế toán!
>
> **[Ôn tập nhanh]**
> Buổi trước chúng ta đã học về Worksheet - công cụ giúp lập BCTC nhanh hơn. Ai còn nhớ 5 bước chuẩn bị worksheet?
>
> **[Gọi sinh viên trả lời]**
> Rất tốt! Trial Balance → Adjustments → Adjusted TB → Extend to IS/BS → Calculate Net Income.
>
> **Hôm nay:** Chúng ta sẽ học một phần CỰC KỲ quan trọng - CLOSING ENTRIES hay còn gọi là "Khóa sổ".
>
> **[Đặt vấn đề]** Các em thử nghĩ xem: Nếu chúng ta không "xóa sổ" các tài khoản doanh thu và chi phí cuối năm, thì sang năm mới sẽ ra sao?
>
> → Đúng rồi! Số liệu của năm cũ và năm mới sẽ lẫn lộn với nhau. Chúng ta sẽ không biết năm nay kinh doanh thế nào!
>
> Đó chính là lý do chúng ta cần Closing Entries.

---

## II. PHẦN 2: CLOSING ENTRIES (120 phút)

### Slide 21-23: Khái niệm và phân loại tài khoản (20 phút)

**Script:**

> **CLOSING THE BOOKS - KHÓA SỔ LÀ GÌ?**
>
> "Closing the books" là quá trình chuẩn bị các tài khoản cho kỳ kế toán tiếp theo. Chúng ta làm điều này bằng cách:
> 1. Chuyển số dư các tài khoản TẠM THỜI về 0
> 2. Chuyển kết quả kinh doanh vào Retained Earnings
>
> **[Vẽ sơ đồ trên bảng]** Hãy tưởng tượng như các em đóng sổ nhật ký cũ và mở sổ mới vào đầu năm.
>
> **MỤC ĐÍCH:**
> - Chuẩn bị cho kỳ kế toán mới
> - Cập nhật Retained Earnings
> - Phân biệt rõ dữ liệu giữa các năm
>
> **[Chuyển sang phân loại tài khoản]**
>
> Để hiểu closing entries, các em PHẢI nắm vững sự khác biệt giữa 2 loại tài khoản:
>
> **TEMPORARY ACCOUNTS (Tạm thời):**
> - Chỉ liên quan đến MỘT kỳ kế toán
> - Bao gồm: TẤT CẢ doanh thu, TẤT CẢ chi phí, và Dividends
> - Được ĐÓNG về 0 cuối kỳ
> - Bắt đầu kỳ mới với số dư = 0
>
> **[Ví dụ]** Service Revenue năm 2020 là 100,000. Sang năm 2021, tài khoản này phải = 0 để tính doanh thu mới cho năm 2021.
>
> **PERMANENT ACCOUNTS (Vĩnh viễn):**
> - Liên quan đến NHIỀU kỳ kế toán
> - Bao gồm: Tài sản, Nợ phải trả, Vốn CSH
> - KHÔNG đóng cuối kỳ
> - Số dư chuyển tiếp sang kỳ sau
>
> **[Ví dụ]** Tiền trong tài khoản Cash cuối năm 2020 sẽ là số dư đầu năm 2021.
>
> **[Hỏi lớp]** Equipment là temporary hay permanent? → Permanent!
> Rent Expense? → Temporary!
> Dividends? → Temporary! (Nhiều em hay nhầm cái này)
>
> **[Nhấn mạnh]** Dividends không phải là chi phí, nhưng nó vẫn là temporary account và được đóng cuối kỳ.

### Slide 24: Income Summary (10 phút)

**Script:**

> Trước khi học 4 bút toán khóa sổ, chúng ta cần biết về một tài khoản đặc biệt: **INCOME SUMMARY**
>
> **Đặc điểm:**
> - Tài khoản TẠM THỜI
> - CHỈ sử dụng trong quá trình khóa sổ
> - KHÔNG xuất hiện trong Trial Balance
> - KHÔNG xuất hiện trong bất kỳ BCTC nào
> - Sau khi khóa sổ → số dư = 0
>
> **Mục đích:** Tập hợp tất cả doanh thu và chi phí trước khi chuyển vào Retained Earnings.
>
> **[Vẽ sơ đồ]**
> ```
> Revenues → Income Summary ← Expenses
>              ↓
>       Retained Earnings
> ```
>
> **[Giải thích]** Tại sao không đóng trực tiếp vào Retained Earnings?
>
> Vì nếu có 20 tài khoản doanh thu và 30 tài khoản chi phí, Retained Earnings sẽ có 50 dòng ghi chú! Quá rối!
>
> Thay vào đó:
> - Đóng 20 revenues → Income Summary
> - Đóng 30 expenses → Income Summary
> - Income Summary có số dư = Net Income
> - Đóng Income Summary → Retained Earnings (chỉ 1 dòng!)
>
> Đơn giản và gọn gàng hơn nhiều phải không các em?

### Slide 25-31: Bốn bút toán khóa sổ (50 phút)

**Script - Tổng quan:**

> Bây giờ đến phần cốt lõi: BỐN BÚT TOÁN KHÓA SỔ
>
> **[Nhấn mạnh]** Các em phải nhớ:
> - Thứ tự RẤT quan trọng: 1 → 2 → 3 → 4
> - KHÔNG được đảo lộn thứ tự
> - Mỗi bút toán có mục đích riêng
>
> Chúng ta sẽ đi chi tiết từng bút toán với ví dụ của Yazici Advertising.

**Script - Bút toán #1:**

> **BÚT TOÁN #1: CLOSE REVENUE ACCOUNTS**
>
> **Mục đích:** Chuyển TẤT CẢ số dư các tài khoản doanh thu vào Income Summary.
>
> **Công thức:**
> ```
> Debit: Revenue Accounts (số dư của chúng)
> Credit: Income Summary (tổng doanh thu)
> ```
>
> **[Giải thích logic]**
> - Revenue accounts có số dư Credit
> - Muốn làm nó về 0 → phải Debit
> - Income Summary nhận Credit (vì doanh thu làm tăng lợi nhuận)
>
> **VÍ DỤ YAZICI:**
>
> Trước khi đóng:
> ```
> Service Revenue
> ----------------
>           | 10,600 (Bal)
> ```
>
> Bút toán khóa sổ:
> ```
> Oct 31  Service Revenue          10,600
>             Income Summary                10,600
> ```
>
> Sau khi đóng:
> ```
> Service Revenue          Income Summary
> ----------------         ----------------
> 10,600 | 10,600                  | 10,600
> -------|-------          
>    0   |                 
> ```
>
> **[Kiểm tra]** Service Revenue có số dư = 0 chư
> **[Gợi ý]**
> - Xác định rõ: Đã ghi gì? Lẽ ra phải ghi gì?
> - So sánh từng tài khoản: Đúng không? Thiếu bao nhiêu? Thừa bao nhiêu?
> - Lập bút toán để chuyển từ SAI → ĐÚNG
>
> Thời gian: 8 phút!
>
> **[Sau 8 phút - Chữa bài]**
>
> **ĐÁP ÁN:**
>
> **Lỗi 1:** Chi phí lương ghi nhầm vào Supplies
> ```
> Salaries and Wages Expense    600
>     Supplies                      600
> ```
> (Chuyển từ Supplies sang Salaries Expense)
>
> **Lỗi 2:** Thu tiền ghi thiếu và sai tài khoản
> ```
> Service Revenue               200
> Cash (3,000-200)            2,800
>     Accounts Receivable       3,000
> ```
> (Đảo Service Revenue, bổ sung Cash thiếu, giảm A/R đúng)
>
> **Lỗi 3:** Mua supplies ghi thiếu số tiền
> ```
> Supplies (860-680)            180
>     Accounts Payable            180
> ```
> (Bổ sung phần thiếu)
>
> **[Giải thích cho sinh viên]**
> Trong thực tế, Current Ratio từ 1.5 - 2.5 được coi là lý tưởng. Công ty này có 2.96 - rất tốt!

---

## IV. TỔNG KẾT TOÀN BỘ CHƯƠNG (10 phút)

**Script:**

> Các em thân mến! Chúng ta đã hoàn thành TRỌN VẸN Chương 4 - một chương RẤT quan trọng!
>
> **[Recap toàn bộ 3 buổi học]**
>
> **BUỔI 1: WORKSHEET**
> - 5 bước chuẩn bị worksheet
> - Công cụ hỗ trợ, không bắt buộc
> - Giúp lập BCTC nhanh hơn
>
> **BUỔI 2: CLOSING ENTRIES**
> - Temporary vs Permanent accounts
> - 4 bút toán khóa sổ (Revenue → Expenses → Income Summary → Dividends)
> - Post-Closing Trial Balance
>
> **BUỔI 3: HÔM NAY**
> - 9 bước Chu trình Kế toán
> - Correcting Entries
> - Classified Balance Sheet
>
> **[Kết nối kiến thức]**
>
> Các em hãy nhìn lại toàn bộ Chu trình Kế toán:
>
> ```
> Chương 2: Analyze → Journalize → Post → Trial Balance
> Chương 3: Adjusting Entries → Adjusted TB
> Chương 4: Financial Statements → Closing → Post-Closing TB
> ```
>
> Một vòng tròn HOÀN HẢO!
>
> **[Ý nghĩa thực tiễn]**
>
> Những gì các em học không chỉ là lý thuyết:
>
> 1. **Trong công việc kế toán:** Đây là quy trình các em sẽ làm HÀNG THÁNG, HÀNG NĂM
>
> 2. **Trong phân tích tài chính:** Classified BS giúp các em đánh giá doanh nghiệp
>
> 3. **Trong kinh doanh:** Hiểu được tài chính để ra quyết định đúng đắn
>
> 4. **Trong Thương mại điện tử:** Các em cần hiểu tài chính để:
>    - Quản lý dòng tiền
>    - Đánh giá hiệu quả kinh doanh online
>    - Phân tích đối thủ cạnh tranh
>    - Lập kế hoạch tài chính cho startup
>
> **[Câu chuyện thực tế]**
>
> Giảng viên kể: "Năm ngoái có một sinh viên khóa trước, sau khi học xong Chương 4 này, em ấy đã giúp gia đình phát hiện ra sai sót trong báo cáo tài chính của công ty gia đình. Nhờ vậy tiết kiệm được hàng trăm triệu đồng thuế!"
>
> **[Thông điệp cuối]**
>
> Kế toán không chỉ là con số. Nó là NGÔN NGỮ của kinh doanh. 
>
> Các em càng hiểu rõ ngôn ngữ này, các em càng có nhiều cơ hội trong sự nghiệp!
>
> **NHỮNG ĐIỂM MẠNH CẦN NHỚ:**
>
> ✅ Worksheet: 5 bước tuần tự, mỗi bước phải cân bằng
> ✅ Closing: 4 bút toán theo đúng thứ tự 1→2→3→4
> ✅ Accounting Cycle: 9 bước bắt buộc + 2 tùy chọn
> ✅ Correcting: So sánh SAI vs ĐÚNG → Lập bút toán điều chỉnh
> ✅ Classified BS: 4 nhóm Assets, 2 nhóm Liabilities, 1 Equity
> ✅ Liquidity: Current Ratio = CA / CL (nên > 1.5)
>
> **[Kỹ năng đã rèn luyện]**
>
> Qua 3 buổi học, các em đã:
> 1. ✓ Tư duy logic (sắp xếp thông tin có hệ thống)
> 2. ✓ Kỹ năng phân tích (phân loại, đánh giá)
> 3. ✓ Chú ý chi tiết (kiểm tra cân bằng)
> 4. ✓ Giải quyết vấn đề (tìm và sửa lỗi)
> 5. ✓ Tư duy phê phán (đánh giá khả năng thanh toán)

---

## V. GIAO BÀI TẬP VÀ HƯỚNG DẪN ÔN TẬP (5 phút)

**Script:**

> **BÀI TẬP VỀ NHÀ - QUAN TRỌNG:**
>
> **1. Bài tập bắt buộc:**
> - E4.1, E4.2, E4.3: Worksheet
> - E4.7, E4.8: Closing Entries
> - E4.12, E4.13: Correcting Entries
> - E4.15, E4.16: Classified Balance Sheet
>
> **2. Bài tập nâng cao (tự nguyện):**
> - P4.1, P4.2: Comprehensive problems
> - ACR4.1: Accounting Cycle Review
>
> **3. Bài tập thực hành trên LMS:**
> - Quiz tự động chấm điểm
> - Case study phân tích Balance Sheet
> - Simulation: Hoàn thành một chu trình kế toán
>
> **CHUẨN BỊ CHO KIỂM TRA:**
>
> **Nội dung kiểm tra sẽ bao gồm:**
>
> 1. **Lý thuyết (30%):**
>    - Định nghĩa các khái niệm
>    - Phân biệt các loại tài khoản
>    - Giải thích quy trình
>
> 2. **Bài tập (70%):**
>    - Lập worksheet hoàn chỉnh
>    - Lập 4 closing entries
>    - Sửa sai sót
>    - Lập Classified Balance Sheet
>    - Phân tích tài chính cơ bản
>
> **HÌNH THỨC KIỂM TRA:**
> - Thời gian: 90 phút
> - Được mang: Máy tính, bút, thước kẻ
> - KHÔNG được mang: Tài liệu, điện thoại
>
> **MẸO ÔN TẬP HIỆU QUẢ:**
>
> **1. Học theo nhóm:**
> - Giải thích cho nhau hiểu
> - Làm bài tập chung
> - Chữa lẫn nhau
>
> **2. Làm bài tập nhiều:**
> - Ít nhất 3 bài mỗi loại
> - Tự đặt đề cho mình
> - Làm lại bài đã sai
>
> **3. Vẽ sơ đồ tư duy:**
> - Sơ đồ Chu trình Kế toán
> - Sơ đồ Closing Process
> - Sơ đồ Classified BS structure
>
> **4. Lập flashcards:**
> - Temporary vs Permanent accounts
> - Các loại tài sản
> - Công thức quan trọng
>
> **5. Thực hành trên Excel:**
> - Tự tạo worksheet template
> - Luyện tập với số liệu khác nhau
>
> **TÀI LIỆU THAM KHẢO:**
>
> 1. **Sách giáo khoa:** Chapter 4 (đọc kỹ phần Summary)
> 2. **Slides bài giảng:** Có trên LMS
> 3. **Video hướng dẫn:** Kênh YouTube của khoa
> 4. **Practice problems:** Sách bài tập kèm theo
> 5. **Website tham khảo:**
>    - AccountingCoach.com
>    - Khan Academy - Accounting
>
> **LỊCH ÔN TẬP ĐỀ NGHỊ:**
>
> **Tuần 1 sau khi học xong:**
> - Ngày 1-2: Ôn Worksheet + làm bài tập
> - Ngày 3-4: Ôn Closing Entries + làm bài tập
> - Ngày 5-6: Ôn Accounting Cycle + Correcting Entries
> - Ngày 7: Ôn Classified BS + làm bài tổng hợp
>
> **Tuần trước kiểm tra:**
> - Làm lại TẤT CẢ bài tập đã làm
> - Làm đề thi thử (có trên LMS)
> - Hỏi các câu còn thắc mắc

---

## VI. Q&A VÀ KẾT THÚC (10 phút)

**Script:**

> **[Mở phần hỏi đáp]**
>
> Bây giờ là thời gian cho các em đặt câu hỏi! Tất cả thắc mắc về Chương 4 đều có thể hỏi.
>
> **CÁC CÂU HỎI THƯỜNG GẶP (dự đoán):**
>
> **Q1: "Thầy/Cô ơi, Worksheet có bắt buộc phải làm không?"**
> 
> A: Không bắt buộc! Nhưng trong bài kiểm tra, nếu đề yêu cầu thì phải làm. Trong thực tế, nhiều công ty nhỏ vẫn dùng để tiện theo dõi.
>
> **Q2: "Tại sao phải đóng Dividends riêng, không đóng chung với Expenses?"**
>
> A: Vì Dividends KHÔNG phải là chi phí! Dividends là việc chia lợi nhuận cho chủ sở hữu, không ảnh hưởng đến tính toán Net Income.
>
> **Q3: "Làm sao để nhớ thứ tự các tài khoản trong Classified BS?"**
>
> A: Nhớ theo logic:
> - Assets: Từ dễ biến thành tiền → khó biến thành tiền
> - Liabilities: Nợ gấp (Current) → Nợ xa (Non-current)
> - Mẹo: "Cash is King" → Cash luôn đứng đầu (hoặc cuối tùy chuẩn)
>
> **Q4: "Nếu quên đóng một expense account thì sao?"**
>
> A: Balance Sheet sẽ không cân bằng! Retained Earnings sẽ sai. Phải phát hiện và sửa ngay.
>
> **Q5: "Correcting Entry có phải làm trước Closing Entry không?"**
>
> A: CÓ! Phải sửa tất cả sai sót TRƯỚC KHI đóng sổ. Nếu không, số liệu chuyển sang năm sau sẽ sai.
>
> **[Trả lời các câu hỏi thực tế từ sinh viên]**
>
> *[Dành 5-7 phút cho học sinh đặt câu hỏi và giải đáp]*
>
> **[Lời kết]**
>
> Cảm ơn các em đã tham gia nhiệt tình trong 3 buổi học!
>
> Chương 4 này là một trong những chương khó nhất của môn Nguyên Lý Kế Toán, nhưng các em đã làm rất tốt!
>
> **NHẮC NHỞ CUỐI CÙNG:**
>
> 1. ✓ Làm hết bài tập về nhà trước buổi sau
> 2. ✓ Tham gia quiz trên LMS (có điểm thưởng!)
> 3. ✓ Hình thành nhóm học tập
> 4. ✓ Liên hệ giảng viên nếu có thắc mắc (email, office hours)
> 5. ✓ Chuẩn bị tốt cho bài kiểm tra
>
> **CHƯƠNG TIẾP THEO:**
>
> Buổi sau chúng ta sẽ học **Chương 5: Accounting for Merchandising Operations**
> - Kế toán cho doanh nghiệp thương mại
> - Inventory systems
> - Purchase và Sales transactions
>
> Rất liên quan đến ngành Thương mại điện tử của các em!
>
> **[Động viên cuối cùng]**
>
> > *"Kế toán không phải là môn học khó. Nó chỉ cần SỰ KIÊN NHẪN và LUYỆN TẬP NHIỀU. Mỗi bài tập các em làm là một bước tiến gần hơn đến thành công!"*
>
> Các em đã làm rất tốt! Thầy/Cô tin rằng các em sẽ đạt kết quả cao trong bài kiểm tra sắp tới.
>
> Hẹn gặp lại các em buổi sau! Chúc các em học tốt và có một tuần làm việc hiệu quả!
>
> **[Vỗ tay động viên]**
>
> 👏👏👏 See you next class! 📚✨

---

# 📎 PHỤ LỤC: THÔNG TIN BỔ SUNG

## A. CHECKLIST CHO GIẢNG VIÊN

### Trước buổi học:
- [ ] Chuẩn bị slides và đảm bảo projector hoạt động
- [ ] In đề bài tập thực hành (nếu cần)
- [ ] Chuẩn bị bảng trắng/bảng đen và marker/phấn
- [ ] Upload tài liệu lên LMS
- [ ] Chuẩn bị ví dụ thực tế (công ty nổi tiếng)
- [ ] Kiểm tra danh sách lớp và điểm danh
- [ ] Chuẩn bị đồng hồ đếm ngược cho bài tập

### Trong buổi học:
- [ ] Điểm danh (5 phút đầu)
- [ ] Review buổi trước (5-10 phút)
- [ ] Tương tác với sinh viên (hỏi-đáp)
- [ ] Cho làm bài tập thực hành
- [ ] Chữa bài chi tiết
- [ ] Tổng kết và giao bài tập

### Sau buổi học:
- [ ] Upload slides lên LMS
- [ ] Đăng đáp án bài tập
- [ ] Trả lời email của sinh viên
- [ ] Chuẩn bị cho buổi sau

---

## B. BẢNG THUẬT NGỮ TIẾNG ANH - TIẾNG VIỆT

| English | Tiếng Việt |
|---------|------------|
| Worksheet | Bảng tính kế toán |
| Closing Entries | Bút toán khóa sổ |
| Temporary Accounts | Tài khoản tạm thời |
| Permanent Accounts | Tài khoản vĩnh viễn |
| Income Summary | Tài khoản tổng hợp thu nhập |
| Post-Closing Trial Balance | Bảng cân đối sau khóa sổ |
| Reversing Entries | Bút toán đảo ngược |
| Correcting Entries | Bút toán điều chỉnh sai sót |
| Accounting Cycle | Chu trình kế toán |
| Classified Balance Sheet | Bảng cân đối kế toán phân loại |
| Current Assets | Tài sản ngắn hạn |
| Non-current Assets | Tài sản dài hạn |
| Property, Plant, & Equipment | Tài sản cố định |
| Intangible Assets | Tài sản vô hình |
| Current Liabilities | Nợ phải trả ngắn hạn |
| Non-current Liabilities | Nợ phải trả dài hạn |
| Working Capital | Vốn lưu động |
| Liquidity | Tính thanh khoản |
| Solvency | Khả năng thanh toán dài hạn |

---

## C. CÔNG THỨC QUAN TRỌNG

### 1. Accounting Equation:
```
Assets = Liabilities + Equity
```

### 2. Net Income:
```
Net Income = Total Revenues - Total Expenses
```

### 3. Retained Earnings:
```
RE (ending) = RE (beginning) + Net Income - Dividends
```

### 4. Working Capital:
```
Working Capital = Current Assets - Current Liabilities
```

### 5. Current Ratio:
```
Current Ratio = Current Assets / Current Liabilities
```

### 6. Debt-to-Equity Ratio:
```
Debt-to-Equity = Total Liabilities / Total Equity
```

---

## D. MẸO GIẢNG DẠY HIỆU QUẢ

### 1. Sử dụng ví dụ thực tế:
- Công ty nổi tiếng (Apple, Samsung, Vinamilk...)
- Tình huống gần gũi với sinh viên (shop online, quán cà phê...)

### 2. Tương tác nhiều:
- Đặt câu hỏi thường xuyên
- Gọi sinh viên trả lời
- Khuyến khích thảo luận nhóm

### 3. Sử dụng công nghệ:
- Kahoot/Quizizz cho quiz
- Mentimeter cho poll
- Excel demo trực tiếp

### 4. Kết nối với thực tế:
- Đọc báo cáo tài chính công ty thật
- Phân tích case study
- Mời guest speaker (kế toán viên thực tế)

### 5. Đa dạng phương pháp:
- Giảng lý thuyết (40%)
- Bài tập thực hành (40%)
- Thảo luận/Q&A (20%)

---

## E. TIÊU CHÍ ĐÁNH GIÁ SINH VIÊN

### Điểm thành phần (100%):
- **Chuyên cần (10%):** Đi học, tham gia hoạt động lớp
- **Bài tập (20%):** Bài tập về nhà, bài tập trên LMS
- **Kiểm tra giữa kỳ (30%):** Chương 1-4
- **Thi cuối kỳ (40%):** Toàn bộ môn học

### Rubric đánh giá bài tập:
- **Hoàn thành đúng hạn (20%)**
- **Chính xác về mặt kỹ thuật (50%)**
- **Trình bày rõ ràng (20%)**
- **Giải thích logic (10%)**

---

**KẾT THÚC SCRIPT BÀI GIẢNG**

*Script này được thiết kế dành cho 3 buổi học × 150 phút, có thể điều chỉnh linh hoạt tùy theo tốc độ lớp học và mức độ tương tác của sinh viên.*

*Chúc giảng viên giảng dạy thành công! 🎓📚*ải thích lỗi thường gặp]**
> - Lỗi 2 là khó nhất: Nhiều em quên đảo Service Revenue
> - Phải nhớ: Service Revenue đã tăng nhầm, phải giảm lại!

---

## III. PHẦN 4: CLASSIFIED BALANCE SHEET (75 phút)

### Slide 51-52: Giới thiệu Classified BS (10 phút)

**Script:**

> **[Chuyển sang phần mới]**
>
> Các em đã biết cách lập Balance Sheet cơ bản. Bây giờ chúng ta sẽ học cách trình bày Balance Sheet theo CHUẨN CHUYÊN NGHIỆP!
>
> **CLASSIFIED STATEMENT OF FINANCIAL POSITION**
>
> **Định nghĩa:** Là bảng cân đối kế toán có NHÓM các tài sản và nợ phải trả tương tự lại với nhau theo các phân loại chuẩn.
>
> **[Hỏi lớp]** Tại sao phải phân loại?
>
> → Để dễ đọc, dễ phân tích, dễ so sánh!
>
> **MỤC ĐÍCH:**
> 1. **Cải thiện khả năng đọc hiểu** - Người dùng nhanh chóng nắm bắt thông tin
> 2. **Hỗ trợ phân tích tài chính** - Dễ tính các tỷ số tài chính
> 3. **Các khoản mục cùng nhóm có đặc điểm tương tự** - Logic và khoa học
> 4. **Xác định khả năng thanh toán** - Đánh giá liquidity
>
> **CÁC PHÂN LOẠI CHUẨN:**
>
> **ASSETS (4 nhóm):**
> 1. Current Assets
> 2. Long-term Investments  
> 3. Property, Plant, and Equipment
> 4. Intangible Assets
>
> **LIABILITIES (2 nhóm):**
> 1. Current Liabilities
> 2. Non-current Liabilities
>
> **EQUITY (1 nhóm):**
> 1. Share Capital & Retained Earnings
>
> **[Nhấn mạnh]** Thứ tự này KHÔNG phải ngẫu nhiên! Nó được sắp xếp theo logic nhất định.

### Slide 53: Current Assets (10 phút)

**Script:**

> **CURRENT ASSETS - TÀI SẢN NGẮN HẠN**
>
> **Định nghĩa:** Tài sản mà công ty dự kiến:
> - CHUYỂN ĐỔI thành tiền, HOẶC
> - SỬ DỤNG HẾT
> 
> Trong vòng **1 NĂM** hoặc **chu kỳ kinh doanh**, tùy khoảng thời gian nào dài hơn.
>
> **[Giải thích chu kỳ kinh doanh]**
>
> Operating Cycle = Thời gian trung bình để:
> ```
> Mua hàng → Bán hàng (trả chậm) → Thu tiền từ khách
> ```
>
> Hầu hết doanh nghiệp: < 1 năm
> Một số ngành đặc biệt (rượu vang, máy bay): > 1 năm
>
> **[Quan trọng]** Nếu không nói gì, mặc định dùng 1 năm!
>
> **THỨ TỰ SẮP XẾP - Theo tính THANH KHOẢN (từ cao → thấp):**
>
> ```
> 1. CASH - Tiền mặt, tiền gửi ngân hàng
>    → Thanh khoản cao nhất!
>
> 2. SHORT-TERM INVESTMENTS - Chứng khoán ngắn hạn
>    → Bán nhanh, thu tiền ngay
>
> 3. ACCOUNTS RECEIVABLE - Phải thu khách hàng
>    → Đợi khách trả tiền
>
> 4. NOTES RECEIVABLE - Phải thu theo hối phiếu
>
> 5. INVENTORY - Hàng tồn kho
>    → Phải bán rồi mới thu tiền
>
> 6. SUPPLIES - Vật tư, công cụ dụng cụ
>
> 7. PREPAID EXPENSES - Chi phí trả trước
>    → Thanh khoản thấp nhất trong nhóm
> ```
>
> **[Ví dụ trình bày]**
> ```
> Current Assets:
>     Cash                        6,600
>     Short-term investments      2,000
>     Accounts receivable         7,000
>     Notes receivable            1,000
>     Inventory                   3,000
>     Supplies                    2,100
>     Prepaid insurance             400
> Total Current Assets           22,100
> ```
>
> **[Mẹo nhớ]** Thứ tự: "Đồ càng dễ biến thành tiền, xếp càng lên trên!"

### Slide 54: Long-term Investments (8 phút)

**Script:**

> **LONG-TERM INVESTMENTS - ĐẦU TƯ DÀI HẠN**
>
> **Định nghĩa:** Tài sản mà công ty KHÔNG dự định chuyển đổi thành tiền trong 1 năm tới.
>
> **[Nhấn mạnh]** Từ khóa: "KHÔNG trong vòng 1 năm"
>
> **BAO GỒM 3 LOẠI:**
>
> **1. Đầu tư tài chính dài hạn:**
> - Cổ phiếu (shares) của công ty khác
> - Trái phiếu (bonds) nắm giữ lâu dài
> - Mục đích: Đầu tư dài hạn, không bán trong năm
>
> **[Ví dụ]** Coca-Cola mua 5% cổ phiếu của Monster Beverage, dự định nắm giữ 5-10 năm.
>
> **2. Tài sản KHÔNG sử dụng trong hoạt động:**
> - Đất để dành xây nhà máy sau 5 năm
> - Tòa nhà cũ không dùng nữa, chờ bán
>
> **[Phân biệt]**
> - Đất đang dùng trong kinh doanh → PPE
> - Đất chờ sử dụng sau → Long-term Investments
>
> **3. Long-term Notes Receivable:**
> - Khoản cho vay dài hạn (> 1 năm)
>
> **[Ví dụ trình bày]**
> ```
> Long-term Investments:
>     Investment in shares of Walters Corp.  5,200
>     Investment in real estate              2,000
> Total Long-term Investments                7,200
> ```

### Slide 55: Property, Plant, and Equipment (10 phút)

**Script:**

> **PROPERTY, PLANT, AND EQUIPMENT (PPE) - TÀI SẢN CỐ ĐỊNH**
>
> Đây là nhóm rất quan trọng - thường chiếm TỶ TRỌNG LỚN trong tổng tài sản!
>
> **Định nghĩa:** Tài sản có:
> - Tuổi thọ tương đối DÀI (> 1 năm)
> - Đang được SỬ DỤNG trong hoạt động kinh doanh
> - Có HÌNH THÁI VẬT CHẤT (nhìn thấy, sờ được)
>
> **BAO GỒM:**
> 1. **LAND** - Đất (đặc biệt: KHÔNG khấu hao!)
> 2. **BUILDINGS** - Nhà xưởng, tòa nhà văn phòng
> 3. **MACHINERY & EQUIPMENT** - Máy móc, thiết bị sản xuất
> 4. **FURNITURE & FIXTURES** - Đồ đạc, trang thiết bị văn phòng
> 5. **VEHICLES** - Xe tải, xe công ty
>
> **[Đặc điểm quan trọng - Cách trình bày]**
>
> PPE được trình bày theo công thức:
> ```
> COST (Giá gốc)
> - ACCUMULATED DEPRECIATION (Khấu hao lũy kế)
> = NET BOOK VALUE (Giá trị còn lại)
> ```
>
> **[Ví dụ chi tiết]**
> ```
> Property, Plant, and Equipment:
>     Land                                    10,000
>     Equipment                    24,000
>     Less: Accumulated depreciation
>           —Equipment            (5,000)     19,000
>     Buildings                    50,000
>     Less: Accumulated depreciation
>           —Buildings           (10,000)     40,000
> Total Property, Plant, and Equipment        69,000
> ```
>
> **[Giải thích]**
> - Equipment cost: 24,000 (giá mua ban đầu)
> - Đã dùng nhiều năm, khấu hao tích lũy: 5,000
> - Giá trị còn lại (Net): 24,000 - 5,000 = 19,000
>
> **[Lưu ý về Land]**
> Tại sao Land không khấu hao?
> → Vì đất không bị hao mòn theo thời gian, thậm chí có thể tăng giá!
>
> **[Phân biệt]**
> - Equipment đang dùng → PPE
> - Equipment dành bán → Inventory
> - Equipment cũ chờ thanh lý → Other Assets

### Slide 56: Intangible Assets (8 phút)

**Script:**

> **INTANGIBLE ASSETS - TÀI SẢN VÔ HÌNH**
>
> Đây là nhóm tài sản "VÔ HÌNH" nhưng cực kỳ GIÁ TRỊ!
>
> **Định nghĩa:** Tài sản có:
> - Tuổi thọ DÀI
> - KHÔNG có hình thái vật chất
> - Thường RẤT có giá trị
>
> **[Ví dụ thực tế]**
> Thương hiệu "Coca-Cola" trị giá hàng chục tỷ USD, nhưng bạn không thể sờ được nó!
>
> **4 LOẠI CHÍNH:**
>
> **1. PATENTS - Bằng sáng chế**
> - Quyền độc quyền sản xuất/bán một sản phẩm
> - Thời hạn: Thường 20 năm
> 
> **[Ví dụ]** Apple có patent cho thiết kế iPhone
>
> **2. COPYRIGHTS - Bản quyền**
> - Quyền sở hữu tác phẩm văn học, nghệ thuật, âm nhạc, phần mềm
> - Thời hạn: Trọn đời tác giả + 70 năm
>
> **[Ví dụ]** Disney sở hữu copyright của Mickey Mouse
>
> **3. TRADEMARKS - Nhãn hiệu thương mại**
> - Tên thương hiệu, logo, slogan độc quyền
> - Thời hạn: Vô thời hạn (nếu tiếp tục sử dụng)
>
> **[Ví dụ]** Logo "swoosh" của Nike, slogan "Just Do It"
>
> **4. GOODWILL - Lợi thế thương mại**
> - Phát sinh khi MUA LẠI doanh nghiệp
> - = Giá mua - Giá trị tài sản ròng
> 
> **[Ví dụ]** Facebook mua Instagram với giá 1 tỷ USD, trong khi tài sản ròng chỉ 100 triệu → Goodwill = 900 triệu
>
> **[Trình bày]**
> ```
> Intangible Assets:
>     Patents                      3,100
>     Goodwill                     6,257
>     Other intangible assets      3,913
> Total Intangible Assets         13,270
> ```
>
> **[Câu hỏi tư duy]** Tại sao Intangible Assets thường được xếp CUỐI CÙNG trong phần tài sản?
>
> → Vì chúng khó định giá và khó chuyển đổi thành tiền nhất!

### Slide 57: Current Liabilities (10 phút)

**Script:**

> Chúng ta đã xong phần Assets. Bây giờ sang phần Liabilities!
>
> **CURRENT LIABILITIES - NỢ PHẢI TRẢ NGẮN HẠN**
>
> **Định nghĩa:** Các khoản nợ mà công ty dự kiến thanh toán trong vòng:
> - 1 NĂM, hoặc
> - Chu kỳ kinh doanh
> (tùy thời gian nào dài hơn)
>
> **[Quan trọng]** Current Liabilities là "nợ cấp bách" - phải trả sớm!
>
> **CÁC KHOẢN MỤC CHÍNH:**
>
> ```
> 1. ACCOUNTS PAYABLE - Phải trả người bán
>    → Mua hàng chưa trả tiền
>
> 2. NOTES PAYABLE (short-term) - Vay ngắn hạn
>    → Vay ngân hàng < 1 năm
>
> 3. SALARIES AND WAGES PAYABLE - Lương phải trả
>    → Đã tính lương nhưng chưa trả
>
> 4. INTEREST PAYABLE - Lãi vay phải trả
>    → Lãi tích lũy chưa trả
>
> 5. INCOME TAXES PAYABLE - Thuế thu nhập phải nộp
>    → Đã tính thuế nhưng chưa nộp
>
> 6. UNEARNED REVENUE - Doanh thu chưa thực hiện
>    → Đã nhận tiền nhưng chưa cung cấp dịch vụ
>
> 7. CURRENT PORTION OF LONG-TERM DEBT
>    → Phần nợ dài hạn đến hạn trong năm
> ```
>
> **[Ví dụ về Current Portion]**
> Công ty vay ngân hàng 100,000 USD, trả trong 5 năm, mỗi năm 20,000.
> - Current Liabilities: 20,000 (trả năm nay)
> - Non-current Liabilities: 80,000 (trả 4 năm sau)
>
> **[Trình bày]**
> ```
> Current Liabilities:
>     Notes payable                    11,000
>     Accounts payable                  2,100
>     Salaries and wages payable        1,600
>     Unearned service revenue            900
>     Interest payable                    450
> Total Current Liabilities            16,050
> ```
>
> **[Phân tích quan trọng - LIQUIDITY]**
>
> **Công thức vàng:**
> ```
> Working Capital = Current Assets - Current Liabilities
> ```
>
> **Ý nghĩa:**
> - Working Capital > 0 → Tốt! Có khả năng trả nợ
> - Working Capital < 0 → Nguy hiểm! Rủi ro thanh khoản
>
> **[Ví dụ]**
> Current Assets: 22,100
> Current Liabilities: 16,050
> Working Capital: 22,100 - 16,050 = 6,050 ✓ (Tốt!)
>
> **Current Ratio** = 22,100 / 16,050 = 1.38
> (Cứ 1 đồng nợ ngắn hạn có 1.38 đồng tài sản ngắn hạn để trả)

### Slide 58: Non-current Liabilities (8 phút)

**Script:**

> **NON-CURRENT LIABILITIES - NỢ DÀI HẠN**
>
> **Định nghĩa:** Các khoản nợ thanh toán SAU 1 NĂM.
>
> **[So sánh]**
> - Current Liabilities: "Nợ gấp" - Phải trả ngay
> - Non-current Liabilities: "Nợ xa" - Trả từ từ
>
> **CÁC KHOẢN MỤC CHÍNH:**
>
> ```
> 1. BONDS PAYABLE - Trái phiếu phải trả
>    → Vay bằng phát hành trái phiếu, kỳ hạn 5-30 năm
>
> 2. MORTGAGES PAYABLE - Nợ thế chấp
>    → Vay mua nhà/đất, thế chấp bằng chính tài sản đó
>
> 3. LONG-TERM NOTES PAYABLE - Vay dài hạn
>    → Hợp đồng vay > 1 năm
>
> 4. LEASE LIABILITIES - Nợ thuê tài chính
>    → Thuê tài sản dài hạn (máy móc, tòa nhà)
>
> 5. PENSION LIABILITIES - Nợ lương hưu
>    → Cam kết trả lương hưu cho nhân viên
>
> 6. DEFERRED TAX LIABILITIES - Thuế hoãn lại
>    → Thuế phải trả trong tương lai
> ```
>
> **[Trình bày]**
> ```
> Non-current Liabilities:
>     Mortgage payable              10,000
>     Notes payable                  1,300
>     Bonds payable                 50,000
>     Pension liabilities            3,500
> Total Non-current Liabilities     64,800
> ```
>
> **[Lưu ý thực tế]**
> Nhiều công ty chỉ báo cáo TỔNG nợ dài hạn trên Balance Sheet, chi tiết được trình bày trong **Notes to Financial Statements**.
>
> **[Câu hỏi]** Tại sao các nhà đầu tư quan tâm đến Non-current Liabilities?
>
> → Vì nó phản ánh:
> - Khả năng thanh toán dài hạn (Solvency)
> - Rủi ro tài chính
> - Chiến lược tài trợ của công ty

### Slide 59: Equity (8 phút)

**Script:**

> **EQUITY - VỐN CHỦ SỞ HỮU**
>
> Phần cuối cùng của Balance Sheet - đại diện cho "của" của chủ sở hữu!
>
> **Định nghĩa:** Phần vốn còn lại sau khi trừ nợ khỏi tài sản.
>
> **Phương trình kế toán:**
> ```
> Assets = Liabilities + Equity
> → Equity = Assets - Liabilities
> ```
>
> **[Giải thích bằng ví dụ đời thường]**
> Các em mua nhà 2 tỷ, vay ngân hàng 1.5 tỷ.
> - Assets (nhà): 2 tỷ
> - Liabilities (nợ): 1.5 tỷ
> - Equity (vốn của bạn): 0.5 tỷ
>
> **THÀNH PHẦN (Corporation):**
>
> **1. SHARE CAPITAL - ORDINARY**
> - Vốn góp của cổ đông
> - Phát hành cổ phiếu
> - KHÔNG thay đổi thường xuyên
> - Chỉ tăng khi phát hành thêm cổ phiếu
>
> **[Ví dụ]** Công ty phát hành 10,000 cổ phiếu, mỗi cổ 100 USD
> → Share Capital = 10,000 × 100 = 1,000,000 USD
>
> **2. RETAINED EARNINGS**
> - Lợi nhuận tích lũy qua các năm
> - CHƯA chia cổ tức
> - Thay đổi mỗi năm
>
> **Công thức:**
> ```
> RE (cuối kỳ) = RE (đầu kỳ) + Net Income - Dividends
> ```
>
> **[Ví dụ]**
> - RE đầu năm: 10,000
> - Net Income năm nay: 5,000
> - Dividends: 2,000
> - RE cuối năm: 10,000 + 5,000 - 2,000 = 13,000
>
> **[Trình bày]**
> ```
> Equity:
>     Share capital—ordinary        20,000
>     Retained earnings             14,050
> Total Equity                      34,050
> ```
>
> **[So sánh với Proprietorship và Partnership]**
> - **Proprietorship:** Chỉ có 1 tài khoản "Owner's Capital"
> - **Partnership:** Mỗi partner có 1 "Capital account"
> - **Corporation:** Share Capital + Retained Earnings
>
> **[Câu hỏi kiểm tra]** Retained Earnings cao có nghĩa là công ty có nhiều tiền mặt không?
>
> → KHÔNG! Retained Earnings là "lợi nhuận để lại", không phải tiền mặt. Tiền có thể đã dùng mua thiết bị, hàng tồn kho...

### Slide 60-61: Ví dụ Classified Balance Sheet hoàn chỉnh (10 phút)

**Script:**

> Bây giờ chúng ta xem một ví dụ HOÀN CHỈNH của Classified Balance Sheet!
>
> **[Chỉ từng phần trên slide]**
>
> **Công ty: Cheng Ltd. - October 31, 2020**
>
> **PHẦN ASSETS:**
>
> **1. Intangible Assets (đầu tiên - theo IFRS):**
> - Patents: 3,100
>
> **2. PPE:**
> - Land: 10,000
> - Equipment (net): 19,000
> - Total PPE: 29,000
>
> **3. Long-term Investments:**
> - Shares: 5,200
> - Real estate: 2,000
> - Total LTI: 7,200
>
> **4. Current Assets (theo thứ tự thanh khoản):**
> - Prepaid insurance: 400
> - Supplies: 2,100
> - Inventory: 3,000
> - Notes receivable: 1,000
> - Accounts receivable: 7,000
> - Short-term investments: 2,000
> - Cash: 6,600
> - Total CA: 22,100
>
> **TOTAL ASSETS: 61,400** ✓
>
> **PHẦN EQUITY & LIABILITIES:**
>
> **1. Equity (đầu tiên - theo IFRS):**
> - Share capital: 20,000
> - Retained earnings: 14,050
> - Total Equity: 34,050
>
> **2. Non-current Liabilities:**
> - Mortgage payable: 10,000
> - Notes payable: 1,300
> - Total NCL: 11,300
>
> **3. Current Liabilities:**
> - Notes payable: 11,000
> - Accounts payable: 2,100
> - Salaries payable: 1,600
> - Unearned revenue: 900
> - Interest payable: 450
> - Total CL: 16,050
>
> **TOTAL EQUITY & LIABILITIES: 61,400** ✓
>
> **[Kiểm tra phương trình kế toán]**
> ```
> Assets = Liabilities + Equity
> 61,400 = (11,300 + 16,050) + 34,050
> 61,400 = 27,350 + 34,050
> 61,400 = 61,400 ✓✓✓
> ```
>
> **[Phân tích nhanh]**
> - Current Ratio: 22,100 / 16,050 = 1.38 (Tốt!)
> - Working Capital: 6,050 (Dương - Tốt!)
> - Debt-to-Equity: 27,350 / 34,050 = 0.80 (Chấp nhận được)
>
> **[Lưu ý về thứ tự trình bày]**
> - **US GAAP:** Assets trước → Liabilities → Equity
> - **IFRS:** Có thể đảo: Non-current trước → Current sau
> - **Việt Nam:** Theo IFRS (từ 2025)

### Slide 62-63: Bài tập tổng hợp (11 phút)

**Script:**

> Bài tập cuối cùng! Các em sẽ tự lập một Classified Balance Sheet hoàn chỉnh.
>
> **[Đọc đề bài]** Công ty GHI có các tài khoản...
>
> **YÊU CẦU:**
> 1. Phân loại từng tài khoản
> 2. Lập Classified BS
> 3. Tính Current Assets và Current Liabilities
> 4. Đánh giá khả năng thanh toán
>
> Thời gian: 10 phút. Làm việc nhóm!
>
> **[Gợi ý]**
> - Xác định từng tài khoản thuộc nhóm nào
> - Nhớ: Equipment và Buildings phải trừ Accumulated Depreciation
> - Kiểm tra: Total Assets = Total Liabilities + Equity
>
> **[Sau 10 phút - Chữa bài]**
>
> **ĐÁP ÁN:**
>
> **Phân loại:**
> - **Intangible:** Patents 5,000
> - **PPE:** Land 50,000 + Buildings 60,000 + Equipment 22,000 = 132,000
> - **Current Assets:** 38,500
> - **Equity:** 122,500
> - **NCL:** 40,000
> - **CL:** 13,000
>
> **Total: 175,500 = 175,500 ✓**
>
> **Tính toán:**
> - Current Assets: 15,000 + 8,000 + 12,000 + 2,000 + 1,500 = **38,500**
> - Current Liabilities: 10,000 + 3,000 = **13,000**
> - Working Capital: 38,500 - 13,000 = **25,500** ✓
> - Current Ratio: 38,500 / 13,000 = **2.96** ✓
>
> **Đánh giá:**
> ✅ Khả năng thanh toán RẤT TỐT!
> - Current Assets gấp gần 3 lần Current Liabilities
> - Working Capital dương mạnh
> - Công ty hoàn toàn có thể trả nợ ngắn hạn
>
> **[Gi**[Kiểm tra]** Service Revenue có số dư = 0 chưa? Có rồi! ✓
>
> Nếu công ty có nhiều tài khoản doanh thu (Service Revenue, Rent Revenue, Interest Revenue...), chúng ta đóng TẤT CẢ cùng một lúc.

**Script - Bút toán #2:**

> **BÚT TOÁN #2: CLOSE EXPENSE ACCOUNTS**
>
> **Mục đích:** Chuyển TẤT CẢ số dư các tài khoản chi phí vào Income Summary.
>
> **Công thức:**
> ```
> Debit: Income Summary (tổng chi phí)
> Credit: Expense Accounts (số dư của từng tài khoản)
> ```
>
> **[Giải thích logic]**
> - Expense accounts có số dư Debit
> - Muốn làm nó về 0 → phải Credit
> - Income Summary bị Debit (vì chi phí làm giảm lợi nhuận)
>
> **VÍ DỤ YAZICI:**
>
> Bút toán khóa sổ:
> ```
> Oct 31  Income Summary           7,740
>             Supplies Expense             1,500
>             Depreciation Expense           40
>             Insurance Expense              50
>             Salaries & Wages Expense    5,200
>             Rent Expense                  900
>             Interest Expense               50
> ```
>
> **[Chỉ vào slide]** Các em thấy không? Tất cả chi phí đều về 0.
>
> Income Summary bây giờ như thế nào?
> ```
> Income Summary
> ----------------
> 7,740 | 10,600 (Revenue)
>       | -------
>       |  2,860 (Số dư Credit)
> ```
>
> **[Hỏi lớp]** Số dư 2,860 Credit này có ý nghĩa gì?
>
> → Đúng rồi! Đó chính là NET INCOME!
>
> Net Income = 10,600 - 7,740 = 2,860 ✓
>
> **[Nhấn mạnh]** Số dư của Income Summary sau bút toán #2 LUÔN LUÔN bằng Net Income (nếu lãi) hoặc Net Loss (nếu lỗ).

**Script - Bút toán #3:**

> **BÚT TOÁN #3: CLOSE INCOME SUMMARY**
>
> **Mục đích:** Chuyển Net Income (hoặc Net Loss) vào Retained Earnings.
>
> **Hai trường hợp:**
>
> **NẾU CÓ LÃI (Income Summary có số dư Credit):**
> ```
> Debit: Income Summary
> Credit: Retained Earnings
> ```
>
> **NẾU CÓ LỖ (Income Summary có số dư Debit):**
> ```
> Debit: Retained Earnings
> Credit: Income Summary
> ```
>
> **[Giải thích]**
> - Lãi làm TĂNG vốn → Credit Retained Earnings
> - Lỗ làm GIẢM vốn → Debit Retained Earnings
>
> **VÍ DỤ YAZICI (có lãi 2,860):**
>
> ```
> Oct 31  Income Summary           2,860
>             Retained Earnings            2,860
> ```
>
> Sau khi ghi sổ:
> ```
> Income Summary          Retained Earnings
> ----------------        ------------------
> 2,860 | Bal 2,860       (đầu kỳ) 0    |
>       | 2,860                           | 2,860
> ------|-------          ----------------
>   0   |                          | 2,860 (Bal)
> ```
>
> **[Kiểm tra]** 
> - Income Summary = 0? ✓
> - Retained Earnings tăng 2,860? ✓
>
> **[Câu hỏi tư duy]** Nếu công ty lỗ 1,000 thì bút toán như thế nào?
>
> → Debit Retained Earnings 1,000 / Credit Income Summary 1,000
>
> Retained Earnings sẽ bị giảm (hoặc âm nếu công ty mới).

**Script - Bút toán #4:**

> **BÚT TOÁN #4: CLOSE DIVIDENDS**
>
> **Mục đích:** Chuyển số dư Dividends vào Retained Earnings.
>
> **Công thức:**
> ```
> Debit: Retained Earnings
> Credit: Dividends
> ```
>
> **[QUAN TRỌNG]** Các em lưu ý 3 điểm:
>
> 1. **Dividends KHÔNG phải là chi phí!**
>    - Không ảnh hưởng Net Income
>    - Không đóng vào Income Summary
>    - Đóng TRỰC TIẾP vào Retained Earnings
>
> 2. **Dividends làm GIẢM vốn**
>    - Vì vậy Debit Retained Earnings
>
> 3. **Dividends được đóng SAU KHI đã đóng Income Summary**
>    - Thứ tự: #1 → #2 → #3 → #4
>
> **VÍ DỤ YAZICI (Dividends = 500):**
>
> ```
> Oct 31  Retained Earnings          500
>             Dividends                     500
> ```
>
> Sau khi ghi sổ:
> ```
> Dividends               Retained Earnings
> ----------------        ------------------
> Bal 500 |               500 | 2,860 (từ #3)
>         | 500           ----|------
> --------|-------             | 2,360 (Bal cuối cùng)
>     0   |                    
> ```
>
> **[Công thức kiểm tra]**
> ```
> Retained Earnings (cuối kỳ) = 0 (đầu kỳ) + 2,860 (NI) - 500 (Div) = 2,360 ✓
> ```
>
> Hoàn hảo! Đây chính là số sẽ xuất hiện trên Balance Sheet.

**Script - Tổng hợp:**

> **[Hiển thị sơ đồ tổng thể]**
>
> Các em hãy nhìn sơ đồ này - đây là toàn bộ quá trình khóa sổ:
>
> ```
> REVENUES (tất cả) ──①──→ INCOME SUMMARY ←──②── EXPENSES (tất cả)
>                              ↓ ③
>                        RETAINED EARNINGS
>                              ↑ ④
>                          DIVIDENDS
> ```
>
> **KẾT QUẢ CUỐI CÙNG:**
> ✅ Tất cả Revenue accounts = 0
> ✅ Tất cả Expense accounts = 0
> ✅ Income Summary = 0
> ✅ Dividends = 0
> ✅ Retained Earnings được cập nhật với NI và Dividends
>
> **[Nhấn mạnh]** Chỉ có các tài khoản TEMPORARY về 0. Các tài khoản PERMANENT giữ nguyên số dư để chuyển sang năm sau.

### Slide 32-33: Post-Closing Trial Balance (15 phút)

**Script:**

> Sau khi đóng sổ xong, chúng ta làm GÌ để kiểm tra?
>
> → Lập POST-CLOSING TRIAL BALANCE!
>
> **ĐỊNH NGHĨA:**
> Post-Closing Trial Balance là bảng cân đối được lập SAU KHI đã ghi nhật ký và chuyển sổ các bút toán khóa sổ.
>
> **MỤC ĐÍCH:**
> 1. Chứng minh Debit = Credit sau khi khóa sổ
> 2. Xác nhận chỉ còn các tài khoản PERMANENT
> 3. Chuẩn bị cho kỳ kế toán mới
>
> **[So sánh]**
>
> **CÓ trong Post-Closing TB:**
> ✅ Cash, Accounts Receivable (tài sản)
> ✅ Accounts Payable, Notes Payable (nợ)
> ✅ Share Capital - Ordinary
> ✅ Retained Earnings (ĐÃ cập nhật)
>
> **KHÔNG CÓ trong Post-Closing TB:**
> ❌ Service Revenue (đã đóng về 0)
> ❌ Salaries Expense (đã đóng về 0)
> ❌ Dividends (đã đóng về 0)
> ❌ Income Summary (luôn = 0)
>
> **VÍ DỤ YAZICI:**
>
> ```
> Yazici Advertising A.Ş.
> Post-Closing Trial Balance
> October 31, 2020
>
>                                     Debit      Credit
> Cash                               15,200
> Accounts Receivable                   200
> Supplies                            1,000
> Equipment                           5,000
> Acc. Depr.—Equipment                             40
> Accounts Payable                                2,500
> Share Capital—Ordinary                          10,000
> Retained Earnings                               2,360
>                                   -------    -------
> Totals                             21,950     21,950
> ```
>
> **[Kiểm tra]**
> - Chỉ có permanent accounts? ✓
> - Debit = Credit? ✓
> - Retained Earnings = 2,360 (như đã tính)? ✓
>
> **[Lưu ý]** Post-Closing Trial Balance này chính là Starting Point (điểm khởi đầu) cho kỳ kế toán tiếp theo!

### Slide 34-36: Bài tập thực hành (25 phút)

**Script:**

> Bây giờ các em sẽ tự mình lập closing entries!
>
> **[Phát đề bài]**
> Công ty DEF có Adjusted Trial Balance tại 31/12/2020...
>
> **HƯỚNG DẪN:**
> 
> Các em làm theo 4 bước:
> 1. Lập bút toán #1: Close revenues (Service Rev + Rent Rev)
> 2. Lập bút toán #2: Close expenses (tất cả expenses)
> 3. Tính Net Income, lập bút toán #3
> 4. Lập bút toán #4: Close Dividends
> 5. Tính Retained Earnings cuối kỳ
> 6. Lập Post-Closing Trial Balance
>
> Thời gian: 15 phút. Các em có thể làm nhóm hoặc cá nhân.
>
> **[Gợi ý]**
> - Tổng Revenues = 50,000
> - Tổng Expenses = 44,000
> - Net Income = ?
>
> **[Sau 15 phút - Chữa bài]**
>
> Được rồi, chúng ta cùng xem đáp án!
>
> **Bút toán #1:**
> ```
> Service Revenue         45,000
> Rent Revenue             5,000
>     Income Summary              50,000
> ```
>
> **Bút toán #2:**
> ```
> Income Summary          44,000
>     Salaries Expense            25,000
>     Rent Expense                12,000
>     Depreciation Expense         3,000
>     Utilities Expense            4,000
> ```
>
> **Bút toán #3:**
> ```
> Income Summary (50,000-44,000)  6,000
>     Retained Earnings                   6,000
> ```
>
> **Bút toán #4:**
> ```
> Retained Earnings        3,000
>     Dividends                    3,000
> ```
>
> **Retained Earnings cuối kỳ:**
> ```
> Beginning: 5,000
> + Net Income: 6,000
> - Dividends: 3,000
> = Ending: 8,000 ✓
> ```
>
> **[Giải thích lỗi thường gặp]**
> 
> Lỗi #1: Đóng Dividends vào Income Summary
> → SAI! Dividends không phải chi phí, đóng trực tiếp vào RE.
>
> Lỗi #2: Quên đóng một số expense
> → Phải đóng TẤT CẢ expenses!
>
> Lỗi #3: Tính sai Retained Earnings
> → Nhớ công thức: Beginning + NI - Dividends = Ending
>
> Có bạn nào còn thắc mắc không?

---

## III. TỔNG KẾT VÀ GIAO BÀI TẬP (10 phút)

**Script:**

> Xuất sắc các em! Chúng ta đã hoàn thành phần Closing Entries.
>
> **ĐIỂM MẠNH CỦA BUỔI HỌC HÔM NAY:**
>
> 1. **Phân biệt được 2 loại tài khoản:**
>    - Temporary: Close về 0
>    - Permanent: Chuyển tiếp
>
> 2. **Nắm vững 4 bút toán khóa sổ:**
>    - #1: Close Revenues
>    - #2: Close Expenses
>    - #3: Close Income Summary
>    - #4: Close Dividends
>
> 3. **Biết lập Post-Closing Trial Balance**
>
> 4. **Hiểu logic:** Tại sao phải đóng sổ?
>    → Để phân biệt dữ liệu giữa các kỳ kế toán!
>
> **[Kết nối với buổi sau]**
>
> Buổi học tiếp theo, chúng ta sẽ:
> - Xem toàn cảnh 9 bước trong Chu trình Kế toán
> - Học cách sửa sai sót (Correcting Entries)
> - Học cách trình bày Classified Balance Sheet chuyên nghiệp
>
> **BÀI TẬP VỀ NHÀ:**
> - Exercises E4.4, E4.7, E4.8 trong sách
> - Làm thêm bài tập closing entries trên LMS
> - Đọc trước về Accounting Cycle (9 steps)
>
> **[Động viên]**
> Các em đã làm rất tốt! Closing Entries là một trong những phần khó nhất nhưng các em đã nắm vững. Hẹn gặp lại các em buổi sau!

---

# 📅 BUỔI HỌC 3: ACCOUNTING CYCLE & CLASSIFIED BALANCE SHEET (150 phút)

## I. REVIEW VÀ GIỚI THIỆU (15 phút)

**Script:**

> Chào mừng các em đến với buổi học cuối cùng của Chương 4!
>
> **[Ôn tập nhanh 2 buổi trước]**
>
> **Buổi 1:** Worksheet - 5 bước chuẩn bị
> **Buổi 2:** Closing Entries - 4 bút toán khóa sổ
>
> **[Mini Quiz - 5 câu nhanh]**
>
> 1. Worksheet có bao nhiêu cột? → 10 cột
> 2. Net Income được tính ở bước nào của worksheet? → Bước 5
> 3. Dividends là temporary hay permanent? → Temporary
> 4. Bút toán nào đóng Dividends? → Bút toán #4
> 5. Post-Closing Trial Balance có chứa Revenue accounts không? → Không!
>
> Rất tốt! Các em đã nắm vững nội dung.
>
> **BUỔI HỌC HÔM NAY:**
>
> Chúng ta sẽ học 2 phần lớn:
>
> **Phần 1 (60 phút):** Toàn cảnh CHU TRÌNH KẾ TOÁN và CORRECTING ENTRIES
> - 9 bước bắt buộc
> - 2 bước tùy chọn
> - Cách sửa sai sót
>
> **Phần 2 (75 phút):** CLASSIFIED BALANCE SHEET
> - Phân loại tài sản
> - Phân loại nợ và vốn
> - Cách trình bày chuyên nghiệp
>
> Đây là buổi tổng hợp, giúp các em có cái nhìn toàn cảnh về kế toán tài chính!

---

## II. PHẦN 3: ACCOUNTING CYCLE & CORRECTING ENTRIES (60 phút)

### Slide 37-40: 9 Bước trong Chu trình Kế toán (20 phút)

**Script:**

> **CHU TRÌNH KẾ TOÁN - THE BIG PICTURE**
>
> Các em đã học rời rạc từng phần. Bây giờ chúng ta sẽ xem toàn cảnh!
>
> **[Vẽ vòng tròn trên bảng]**
>
> Chu trình kế toán là chuỗi các bước được thực hiện LIÊN TỤC trong mỗi kỳ kế toán.
>
> **9 BƯỚC BẮT BUỘC:**
>
> **BƯỚC 1: ANALYZE BUSINESS TRANSACTIONS**
> - Phân tích các giao dịch kinh doanh
> - Xác định tài khoản nào bị ảnh hưởng
> - Xác định tăng hay giảm
>
> **[Ví dụ]** Bán hàng thu tiền mặt 1,000
> → Cash tăng, Revenue tăng
>
> **BƯỚC 2: JOURNALIZE THE TRANSACTIONS**
> - Ghi vào sổ nhật ký tổng hợp
> - Theo thứ tự thời gian
> - Có giải thích
>
> **BƯỚC 3: POST TO LEDGER ACCOUNTS**
> - Chuyển từ journal vào ledger
> - Cập nhật số dư từng tài khoản
>
> **[Nhấn mạnh]** Bước 1-3 có thể làm HÀNG NGÀY trong kỳ kế toán.
>
> **BƯỚC 4: PREPARE A TRIAL BALANCE**
> - Lập bảng cân đối số phát sinh
> - Kiểm tra Debit = Credit
> - Phát hiện sai sót ghi sổ
>
> **[Hỏi lớp]** Nếu Trial Balance cân bằng, có nghĩa là không có lỗi à?
> → Không! Vẫn có thể có lỗi (ví dụ: ghi nhầm tài khoản, ghi sai số tiền nhưng vẫn cân bằng)
>
> **BƯỚC 5: JOURNALIZE AND POST ADJUSTING ENTRIES**
> - Ghi nhật ký các bút toán điều chỉnh
> - Chuyển sổ vào ledger
> - Đây là nội dung Chương 3!
>
> **BƯỚC 6: PREPARE ADJUSTED TRIAL BALANCE**
> - Lập bảng cân đối SAU điều chỉnh
> - Đảm bảo tất cả adjustments đã được ghi
>
> **BƯỚC 7: PREPARE FINANCIAL STATEMENTS**
> - Income Statement
> - Retained Earnings Statement
> - Balance Sheet
> - Statement of Cash Flows
>
> **[Nhấn mạnh]** Bước 4-7 thường làm ĐỊNH KỲ (tháng, quý, năm).
>
> **BƯỚC 8: JOURNALIZE AND POST CLOSING ENTRIES**
> - 4 bút toán khóa sổ (học ở buổi 2)
> - Đóng temporary accounts về 0
>
> **BƯỚC 9: PREPARE POST-CLOSING TRIAL BALANCE**
> - Kiểm tra sau khi đóng sổ
> - Chỉ có permanent accounts
> - Chuẩn bị cho kỳ mới
>
> **[Nhấn mạnh]** Bước 8-9 thường chỉ làm CUỐI NĂM.
>
> **[Hiển thị sơ đồ tổng thể]**
>
> Các em thấy không? Đây là một vòng tròn khép kín:
> ```
> Analyze → Journalize → Post → Trial Balance → 
> Adjust → Adjusted TB → Financial Statements → 
> Close → Post-Closing TB → [Quay lại Analyze cho kỳ mới]
> ```

### Slide 41: Optional Steps (10 phút)

**Script:**

> Ngoài 9 bước bắt buộc, có 2 bước TÙY CHỌN:
>
> **1. WORKSHEET**
> - Vị trí: Giữa bước 4 và 7
> - Lợi ích: Lập BCTC nhanh hơn
> - Không bắt buộc phải làm
>
> **2. REVERSING ENTRIES**
> - Vị trí: Ngày đầu tiên của kỳ mới (sau bước 9)
> - Lợi ích: Đơn giản hóa ghi chép kỳ sau
> - Chỉ áp dụng cho một số adjustments
>
> **[Giải thích Reversing Entries]**
>
> Đây là kỹ thuật nâng cao, chúng ta sẽ học ví dụ đơn giản:
>
> **Tình huống:** Ngày 31/10 accrued salaries 1,200
> ```
> Oct 31  Salaries Expense       1,200
>             Salaries Payable           1,200
> ```
>
> **Ngày 1/11 (đầu tháng mới), làm Reversing Entry:**
> ```
> Nov 1   Salaries Payable       1,200
>             Salaries Expense           1,200
> ```
> (Hoàn toàn ngược lại!)
>
> **Ngày 9/11 trả lương 4,000:**
>
> **Nếu KHÔNG dùng reversing:**
> ```
> Salaries Payable    1,200
> Salaries Expense    2,800
>     Cash                    4,000
> ```
> (Phải nhớ có 1,200 accrued từ tháng trước!)
>
> **Nếu DÙNG reversing:**
> ```
> Salaries Expense    4,000
>     Cash                    4,000
> ```
> (Đơn giản hơn! Không cần nhớ accrual)
>
> **[Nhấn mạnh]** Reversing entries KHÔNG thay đổi kết quả, chỉ giúp ghi chép đơn giản hơn.

### Slide 42-50: Correcting Entries (30 phút)

**Script - Giới thiệu:**

> Bây giờ chúng ta đến phần RẤT thực tế: **CORRECTING ENTRIES**
>
> Trong thực tế làm kế toán, chúng ta KHÔNG THỂ TRÁNH KHỎI sai sót:
> - Ghi sai số tiền
> - Ghi nhầm tài khoản
> - Quên ghi một phần
> - Đảo Debit/Credit
>
> **[Hỏi lớp]** Các em nghĩ sao nếu phát hiện sai sót sau khi đã ghi sổ?
>
> → Phải làm CORRECTING ENTRY để sửa!
>
> **PHÂN BIỆT:**
>
> | Tiêu chí | Adjusting Entries | Correcting Entries |
> |----------|-------------------|-------------------|
> | Mục đích | Cập nhật theo accrual basis | Sửa SAI SÓT |
> | Tần suất | Bắt buộc cuối kỳ | Khi phát hiện lỗi |
> | Thời điểm | Cuối kỳ | Bất cứ lúc nào |
> | Tài khoản | 1 BS + 1 IS | Bất kỳ |
>
> **[Nhấn mạnh]** Correcting entries chỉ cần làm khi CÓ SAI SÓT!

**Script - Các loại sai sót:**

> **4 LOẠI SAI SÓT THƯỜNG GẶP:**
>
> **LOẠI 1: SAI SỐ TIỀN**
> ```
> Thực tế: Thu tiền 950
> Đã ghi: Debit Cash 590 (ghi thiếu)
> ```
>
> **LOẠI 2: SAI TÀI KHOẢN**
> ```
> Thực tế: Chi phí quảng cáo 75
> Đã ghi: Debit Miscellaneous Expense 75
> Đúng ra: Debit Advertising Expense 75
> ```
>
> **LOẠI 3: THIẾU BÚT TOÁN**
> ```
> Thực tế: Trả lương 1,900 (bao gồm 700 accrued từ trước)
> Đã ghi: Debit Salaries Expense 1,900
> Thiếu: Phải debit Salaries Payable 700
> ```
>
> **LOẠI 4: GHI NHẦM DEBIT/CREDIT**
> ```
> Mua thiết bị 310
> Đã ghi: Debit Supplies 310
> Đúng ra: Debit Equipment 310
> ```

**Script - Phương pháp lập Correcting Entry:**

> **3 BƯỚC LẬP CORRECTING ENTRY:**
>
> **BƯỚC 1:** Xác định bút toán SAI (đã ghi gì?)
> **BƯỚC 2:** Xác định bút toán ĐÚNG (lẽ ra phải ghi gì?)
> **BƯỚC 3:** Lập bút toán điều chỉnh (từ SAI → ĐÚNG)
>
> **[Công thức]**
> ```
> Correcting Entry = What SHOULD BE - What WAS recorded
> ```
>
> **VÍ DỤ 1: SAI TÀI KHOẢN**
>
> Ngày 10/5: Thu tiền từ khách 500, nhưng ghi:
> ```
> SAI (đã ghi):
> Cash                    500
>     Service Revenue         500
>
> ĐÚNG (lẽ ra phải ghi):
> Cash                    500
>     Accounts Receivable     500
> ```
>
> **Phân tích:**
> - Cash: Đúng rồi, không cần sửa
> - Service Revenue: Đã tăng nhầm, phải GIẢM (Debit)
> - Accounts Receivable: Lẽ ra phải giảm, chưa giảm (Credit)
>
> **CORRECTING ENTRY (20/5):**
> ```
> Service Revenue         500
>     Accounts Receivable     500
> ```
>
> **[Giải thích]** Chúng ta "đảo ngược" phần sai và "làm đúng" phần đúng.
>
> **VÍ DỤ 2: SAI SỐ TIỀN**
>
> Ngày 18/5: Mua thiết bị 4,500, nhưng ghi thành 450:
> ```
> SAI (đã ghi):
> Equipment               450
>     Accounts Payable        450
>
> ĐÚNG (lẽ ra phải ghi):
> Equipment             4,500
>     Accounts Payable      4,500
> ```
>
> **Phân tích:**
> - Equipment: Thiếu 4,050
> - Accounts Payable: Thiếu 4,050
>
> **CORRECTING ENTRY (3/6):**
> ```
> Equipment             4,050
>     Accounts Payable      4,050
> ```
>
> **[Nhấn mạnh]** Chỉ cần bổ sung phần THIẾU!

**Script - Bài tập thực hành:**

> **BÀI TẬP:** Công ty XYZ có 3 lỗi trong tháng 1/2020.
>
> Các em hãy lập correcting entry cho từng lỗi.
>
> **[Gợi ý]**
> - Xác định rõ: Đã ghi gì? Lẽ ra phải ghi gì?
> - So sánh từng tài khoản:
